...
import { 
  ...
  Folder, FolderOpen, ChevronDown, ChevronRight, ChevronUp
} from 'lucide-react';
...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="min-h-screen flex bg-brand-green text-bg-cream/90 font-light">
      ...
      <aside className="w-64 bg-brand-green border-r border-bg-beige/10 px-5 py-6 flex flex-col shrink-0 h-screen sticky top-0 gap-4">
        <div className="flex flex-col flex-1 min-h-0 gap-6 overflow-y-auto [&::-webkit-scrollbar]:hidden">
          ...
          <nav className="flex flex-col gap-2">
            ...
            >
              ...
              <span className="whitespace-nowrap">Orders Management</span>
            </button>

            {/* COLLAPSIBLE CONTENT MANAGER SECTION */}
            <div className="flex flex-col">
              <button
                onClick={() => setIsContentManagerOpen(!isContentManagerOpen)}
                className="flex items-center gap-3 text-xs uppercase font-bold tracking-wider px-4 py-3 rounded-xl transition-all cursor-pointer text-bg-cream/70 hover:bg-bg-beige/5 hover:text-bg-cream text-left w-full"
              >
                {isContentManagerOpen ? (
                  <FolderOpen className="w-4 h-4 shrink-0 text-brand-gold" />
                ) : (
                  <Folder className="w-4 h-4 shrink-0 text-brand-gold" />
                )}
                <span className="whitespace-nowrap">Content Manager</span>
                {isContentManagerOpen ? (
                  <ChevronUp className="w-4 h-4 shrink-0 text-brand-gold ml-auto" />
                ) : (
                  <ChevronDown className="w-4 h-4 shrink-0 text-brand-gold ml-auto" />
                )}
              </button>

              {isContentManagerOpen && (
                <div className="flex flex-col gap-1.5 pl-4 border-l border-bg-beige/10 ml-5 my-1">
                  <button
                  ...
                  >
                    ...
                    <span className="whitespace-nowrap">Material Blog</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="whitespace-nowrap">Wellness Benefits</span>
                  </button>
            ...
            >
              <Inbox className="w-4 h-4 shrink-0" />
              <span className="whitespace-nowrap">B2B Inquiries</span>
            ...
            {adminUser?.role === 'super_admin' && (
              <button
                ...
                className={`flex items-center gap-3 text-xs uppercase font-bold tracking-wider px-4 py-3 rounded-xl transition-all cursor-pointer ${
                  activeTab === 'super' 
                    ? 'bg-red-900/80 text-red-100 shadow' 
                    : 'text-bg-cream/70 hover:bg-bg-beige/5 hover:text-bg-cream'
                }`}
              >
        ...
        {/* User logout section — always pinned at bottom */}
        <div className="flex flex-col gap-4 pt-4 border-t border-bg-beige/10 shrink-0">
      ...
      <main className="flex-grow p-8 sm:p-10 flex flex-col gap-8 h-screen overflow-y-auto w-full max-w-[calc(100vw-256px)] bg-brand-green/98">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            {/* Quick Search inside panel — hidden for orders & super admin */}
            {activeTab !== 'orders' && activeTab !== 'super' && (
            ...
            {activeTab === 'materials' && (
              <button
                ...
                className="bg-brand-gold hover:bg-brand-gold/90 text-brand-green font-bold text-xs uppercase tracking-wider px-4 py-2 rounded-xl flex items-center gap-1.5 cursor-pointer shadow"
              >
                <Plus className="w-4 h-4" /> Add Material Blog
              </button>
            )}
        ...
        <div className="flex-grow w-full">
          ...
          {activeTab === 'inquiries' && (
            <div className="flex flex-col gap-6 w-full">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start w-full">
                ...
                {/* Table list (Left) */}
                <div className="lg:col-span-8 bg-bg-cream/5 border border-bg-beige/10 rounded-2xl overflow-hidden luxury-shadow max-h-[520px] overflow-y-auto [&::-webkit-scrollbar]:hidden">
                  <table className="w-full border-collapse text-left text-xs">
                    ...
                    <tbody className="divide-y divide-bg-beige/10">
                      {filteredInquiries
                        .filter((inq) => inq.inquiry_type !== 'order')
                        .map((inq) => (
                          <tr
                            key={inq.id}
                      ...
                      {filteredInquiries.filter((inq) => inq.inquiry_type !== 'order').length === 0 && (
                        <tr>
          ...
          {/* TAB 2: PRODUCTS CRUD */}
          {activeTab === 'products' && (
            <div className="bg-bg-cream/5 border border-bg-beige/10 rounded-2xl overflow-hidden luxury-shadow max-h-[520px] overflow-y-auto [&::-webkit-scrollbar]:hidden">
              <table className="w-full border-collapse text-left text-xs">
                <thead>
                  <tr className="bg-brand-green/95 border-b border-bg-beige/10 text-brand-gold font-bold uppercase tracking-wider sticky top-0 z-10">
          ...
          {activeTab === 'super' && adminUser?.role === 'super_admin' && (
            <SuperAdminView />
          )}
          ...
          {activeTab === 'users' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full">
              ...
              {/* LEFT — Users List */}
              <div className={`${selectedUser ? 'lg:col-span-7' : 'lg:col-span-12'} bg-bg-cream/5 border border-bg-beige/10 rounded-2xl overflow-hidden luxury-shadow flex flex-col transition-all duration-200`}>

                {/* Column headers */}
                <div className="grid grid-cols-12 px-4 py-3 border-b border-bg-beige/10 bg-brand-green/95 sticky top-0 z-10">
                  <span className="col-span-4 text-[9px] uppercase font-bold tracking-wider text-brand-gold">Customer</span>
                  <span className="col-span-4 text-[9px] uppercase font-bold tracking-wider text-brand-gold">Contact</span>
                  <span className="col-span-2 text-[9px] uppercase font-bold tracking-wider text-brand-gold">Joined</span>
                  <span className="col-span-2 text-[9px] uppercase font-bold tracking-wider text-brand-gold text-right">Actions</span>
                </div>

                {/* Scrollable rows */}
                <div className="overflow-y-auto [&::-webkit-scrollbar]:hidden max-h-[520px] divide-y divide-bg-beige/10">
                  {filteredUsers.length === 0 ? (
                    <p className="text-bg-cream/40 text-xs text-center py-12">No customers found.</p>
                  ) : (
                    filteredUsers.map((u) => (
                      <button
                        key={u.id}
                        ...
                        className={`w-full grid grid-cols-12 px-4 py-3.5 text-left transition-all hover:bg-bg-beige/5 text-xs ${
                          selectedUser?.id === u.id ? 'bg-bg-beige/5 border-l-2 border-brand-gold' : ''
                        }`}
                      >
                        <span className="col-span-4 flex items-center gap-2.5 text-bg-cream font-semibold truncate">
                          <div className="w-7 h-7 rounded-full bg-brand-gold/20 flex items-center justify-center font-serif text-xs font-bold text-brand-gold shrink-0">
                            {(u.name?.[0] || u.phone?.[0] || 'U').toUpperCase()}
                          </div>
                          <span className="truncate">{u.name || 'Incomplete'}</span>
                        </span>
                        <span className="col-span-4 text-bg-cream/70 truncate self-center">
                          {u.phone || u.email || <span className="text-bg-cream/30 italic">No contact</span>}
                        </span>
                        <span className="col-span-2 text-bg-cream/50 text-[10px] self-center">
                          {u.created_at ? new Date(u.created_at).toLocaleDateString() : 'N/A'}
                        </span>
                        <span className="col-span-2 flex justify-end items-center" onClick={e => e.stopPropagation()}>
                          <button
                            onClick={() => handleDeleteUser(u.id)}
                            className="text-red-400 hover:text-red-300 p-1.5 rounded-lg hover:bg-red-500/5 transition-colors cursor-pointer"
                            title="Delete"
                          >
              ...
              {/* RIGHT — User Detail (only when selected) */}
              {selectedUser && (
                <div className="lg:col-span-5 bg-bg-cream/5 border border-bg-beige/10 rounded-2xl luxury-shadow flex flex-col">
                  {/* Header */}
                  <div className="px-5 py-4 border-b border-bg-beige/10 flex items-start justify-between shrink-0">
                    <div>
                      <span className="text-[9px] uppercase font-bold tracking-widest text-brand-gold block">Customer Profile</span>
                      <h3 className="font-serif text-base font-bold text-bg-cream mt-0.5">
                        {selectedUser.name || 'Profile Incomplete'}
                      </h3>
                    </div>
                    <button onClick={() => setSelectedUser(null)} className="text-bg-cream/40 hover:text-bg-cream transition-colors">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                  ...
                  <div className="p-5 flex flex-col gap-5 overflow-y-auto [&::-webkit-scrollbar]:hidden flex-1">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-full bg-brand-gold/20 flex items-center justify-center font-serif text-2xl font-bold text-brand-gold shrink-0">
                        {(selectedUser.name?.[0] || selectedUser.phone?.[0] || 'U').toUpperCase()}
                      </div>
                      <div>
                        <p className="font-bold text-bg-cream text-sm">{selectedUser.name || 'No Name'}</p>
                        <p className="text-[10px] text-bg-cream/50 uppercase tracking-wider mt-0.5">ID: {selectedUser.id}</p>
                        <span className="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border bg-green-950/60 text-green-200 border-green-500/20 mt-1 inline-block">
                          {selectedUser.role}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-bg-beige/5 rounded-xl p-4 flex flex-col gap-3 border border-bg-beige/10">
                      ...
                      {selectedUser.phone && (
                        <div className="flex items-center gap-2 text-xs text-bg-cream/80">
                          <span className="text-brand-gold text-[10px] font-bold uppercase tracking-wider w-12 shrink-0">Phone</span>
                          <span className="font-medium">{selectedUser.phone}</span>
                        </div>
                      )}
                      {selectedUser.email && (
                        <div className="flex items-center gap-2 text-xs text-bg-cream/80">
                          <span className="text-brand-gold text-[10px] font-bold uppercase tracking-wider w-12 shrink-0">Email</span>
                          <span className="font-medium truncate">{selectedUser.email}</span>
                        </div>
                      )}
                      {!selectedUser.phone && !selectedUser.email && (
                        <p className="text-bg-cream/30 text-xs italic">No contact details recorded</p>
                      )}
                    </div>
                    ...
                    {(selectedUser.address || selectedUser.city) && (
                      <div className="bg-bg-beige/5 rounded-xl p-4 flex flex-col gap-3 border border-bg-beige/10">
                        <span className="text-[9px] uppercase font-bold tracking-widest text-brand-gold">Delivery Address</span>
                        <p className="text-xs text-bg-cream/80 leading-relaxed">
                          {[selectedUser.address, selectedUser.city, selectedUser.pinCode, selectedUser.country].filter(Boolean).join(', ')}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between border-t border-bg-beige/10 pt-4">
                      <span className="text-[10px] text-bg-cream/50 uppercase tracking-wider">Registered</span>
                      <span className="text-xs text-bg-cream/70">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-green/80 backdrop-blur-sm p-4 overflow-y-auto" onClick={() => setShowProductModal(false)}>
          <div className="bg-bg-cream text-text-primary rounded-3xl p-6 sm:p-8 max-w-2xl w-full border border-brand-green/10 shadow-2xl relative max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleProductSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-xs">
              ...
              <div className="flex flex-col sm:col-span-2 pt-2 border-t border-brand-green/5">
                <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-2.5 pl-1 block">Linked Material Blogs</label>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-bg-beige/25 p-4 rounded-xl border border-brand-green/10">
                  {materials.map((mat) => (
              ...
              <div className="flex flex-col sm:col-span-2 pt-2 border-t border-brand-green/5">
                ...
                <p className="text-[10px] text-text-muted mb-3 pl-1">Check a benefit to assign it. A custom description field will appear â€” leave blank to use the default.</p>
                      ...
                      <div key={ben.id} className={`rounded-xl border transition-all duration-200 overflow-hidden ${
                        ...
                        {isChecked && (
                          <div className="px-3 pb-3">
                            <label className="text-[9px] uppercase font-bold tracking-wider text-brand-green/70 block mb-1">Custom description for this product <span className="text-text-muted font-normal normal-case">(optional â€” overrides default)</span></label>
                            <textarea
                              rows={2}
              ...
              <div className="flex flex-col sm:col-span-2 pt-2 border-t border-brand-green/5">
                ...
                <p className="text-[10px] text-text-muted mb-3 pl-1">Check an application to assign it. A custom description field will appear â€” leave blank to use the default.</p>
                <div className="flex flex-col gap-3">
                  {usageTypes.map((ut) => {
      ...
      {showBlogModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-green/80 backdrop-blur-sm p-4 overflow-y-auto" onClick={() => setShowBlogModal(false)}>
          <div className="bg-bg-cream text-text-primary rounded-3xl p-6 sm:p-8 max-w-2xl w-full border border-brand-green/10 shadow-2xl relative max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleBlogSubmit} className="flex flex-col gap-8 text-xs">

              {/* â”€â”€ SECTION 1: Identity â”€â”€ */}
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <span className="w-5 h-5 rounded-full bg-brand-green text-bg-cream flex items-center justify-center text-[10px] font-bold shrink-0">1</span>
                  <span className="text-[11px] uppercase font-bold tracking-widest text-brand-green">Diary Identity</span>
                  <div className="flex-1 h-px bg-brand-green/10 ml-1" />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-1">Web Path Slug *</label>
                  ...
                  <div className="flex flex-col">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-1">Author Name *</label>
              ...
              {/* â”€â”€ SECTION 2: Publishing Details â”€â”€ */}
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <span className="w-5 h-5 rounded-full bg-brand-green text-bg-cream flex items-center justify-center text-[10px] font-bold shrink-0">2</span>
                  <span className="text-[11px] uppercase font-bold tracking-widest text-brand-green">Publishing Details</span>
                  <div className="flex-1 h-px bg-brand-green/10 ml-1" />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="flex flex-col">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-1">Publish Date *</label>
                    <input type="date" required name="publish_date"
                      defaultValue={editingBlog?.publish_date || new Date().toISOString().split('T')[0]}
                      className="bg-bg-beige/30 p-2.5 rounded-lg border border-brand-green/10 focus:outline-none focus:border-brand-gold text-text-primary"
                    />
                  </div>
                  <div className="flex flex-col">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-1">Category</label>
                    <select name="category" defaultValue={editingBlog?.category || 'Organic Living'}
                      className="bg-bg-beige/30 p-2.5 rounded-lg border border-brand-green/10 focus:outline-none focus:border-brand-gold text-text-primary">
                      <option value="Organic Living">Organic Living</option>
                      <option value="Natural Living">Natural Living</option>
                      <option value="Harvest Journal">Harvest Journal</option>
                      <option value="Sustainability">Sustainability</option>
                    </select>
                  </div>
                  <div className="flex flex-col">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-1">Status</label>
                    <select name="status" defaultValue={editingBlog?.status || 'published'}
                      className="bg-bg-beige/30 p-2.5 rounded-lg border border-brand-green/10 focus:outline-none focus:border-brand-gold text-text-primary">
                  ...
                  <div className="flex flex-col sm:col-span-3">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-1">Tags (Comma separated)</label>
                    <input type="text" name="tags"
                      defaultValue={editingBlog?.tags || 'harvest, saffron, pampore'}
                      placeholder="e.g. saffron, harvest, organic, pampore"
                      className="bg-bg-beige/30 p-2.5 rounded-lg border border-brand-green/10 focus:outline-none focus:border-brand-gold"
                    />
                  </div>
                </div>
              ...
              {/* â”€â”€ SECTION 3: Content â”€â”€ */}
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <span className="w-5 h-5 rounded-full bg-brand-green text-bg-cream flex items-center justify-center text-[10px] font-bold shrink-0">3</span>
                  <span className="text-[11px] uppercase font-bold tracking-widest text-brand-green">Content & SEO</span>
                  <div className="flex-1 h-px bg-brand-green/10 ml-1" />
                </div>
                <div className="flex flex-col gap-4">
                  <div className="flex flex-col">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-1">SEO Teaser / Meta Description *</label>
                  ...
                  <div className="flex flex-col">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-1">
                      Main Editorial Content *
                      <span className="ml-2 text-text-muted normal-case font-normal">Use ## for headings, **text** for bold</span>
                    </label>
                    <textarea required name="content" rows={8}
                      defaultValue={editingBlog?.content || ''}
                      placeholder="Write full article content here. Use '## Subtitle' for section headers and '**word**' for bold text..."
                      className="bg-bg-beige/30 p-3 rounded-lg border border-brand-green/10 focus:outline-none focus:border-brand-gold resize-none"
                    ></textarea>
                  </div>
                </div>
              ...
              {/* â”€â”€ SECTION 4: Media â”€â”€ */}
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <span className="w-5 h-5 rounded-full bg-brand-green text-bg-cream flex items-center justify-center text-[10px] font-bold shrink-0">4</span>
                  <span className="text-[11px] uppercase font-bold tracking-widest text-brand-green">Cover Media</span>
                  <div className="flex-1 h-px bg-brand-green/10 ml-1" />
                </div>
                <div className="flex flex-col">
                  <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-1">Cover Image URL</label>
              ...
              {/* â”€â”€ SECTION 5: Relations â”€â”€ */}
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <span className="w-5 h-5 rounded-full bg-brand-green text-bg-cream flex items-center justify-center text-[10px] font-bold shrink-0">5</span>
                  <span className="text-[11px] uppercase font-bold tracking-widest text-brand-green">Related Content</span>
                  <div className="flex-1 h-px bg-brand-green/10 ml-1" />
                </div>
                <div className="flex flex-col gap-4">
                  <div className="flex flex-col">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-2">Related Products (Showcase)</label>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-bg-beige/25 p-4 rounded-xl border border-brand-green/10">
                      {products.map((prod) => (
                        <label key={prod.id} className="flex items-center gap-2 text-xs text-text-primary cursor-pointer hover:text-brand-green transition-colors py-0.5">
                          <input type="checkbox" checked={selectedBlogProductIds.includes(prod.id)}
                            onChange={(e) => {
                              if (e.target.checked) setSelectedBlogProductIds(prev => [...prev, prod.id]);
                              else setSelectedBlogProductIds(prev => prev.filter(id => id !== prod.id));
                            }}
                            className="w-4 h-4 text-brand-gold accent-brand-green"
                          />
                          <span> {prod.name}</span>
                  ...
                  <div className="flex flex-col">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-brand-green mb-2">Related Material Blogs</label>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-bg-beige/25 p-4 rounded-xl border border-brand-green/10">
                      {materials.map((mat) => (
                        <label key={mat.id} className="flex items-center gap-2 text-xs text-text-primary cursor-pointer hover:text-brand-green transition-colors py-0.5">
                          <input type="checkbox" checked={selectedBlogMaterialIds.includes(mat.id)}
                            onChange={(e) => {
                              if (e.target.checked) setSelectedBlogMaterialIds(prev => [...prev, mat.id]);
                              else setSelectedBlogMaterialIds(prev => prev.filter(id => id !== mat.id));
                            }}
                            className="w-4 h-4 text-brand-gold accent-brand-green"
                          />
                          <span>{mat.name}</span>
              ...
              <button type="submit"
                className="bg-brand-green hover:bg-brand-gold hover:text-brand-green text-bg-cream font-bold text-xs uppercase tracking-wider py-3.5 rounded-xl mt-2 cursor-pointer shadow"
              >
                Save Editorial Diary
              </button>
